...
class GINConv(MessagePassing):
    def __init__(self, emb_dim, encode_edge=True, edge_dim=None):
        ...
        if encode_edge:
            ...
            self.bond_encoder = torch.nn.Linear(edge_dim, emb_dim)
        else:
...
class GNN_node(torch.nn.Module):
    ...
    """
    def __init__(self, num_layer, emb_dim, drop_ratio = 0.5, JK = "last", residual = False, gnn_name = 'gin', atom_encode=True, node_dim=None, edge_dim=None):
        '''
            emb_dim (int): node embedding dimensionality
        ...
        for layer in range(num_layer):
            if gnn_name == 'gin':
                self.convs.append(GINConv(emb_dim, encode_edge=encode_edge, edge_dim=edge_dim))
            elif gnn_name == 'gcn':
                self.convs.append(GCNConv(emb_dim, encode_edge=encode_edge, edge_dim=edge_dim))
            else:
